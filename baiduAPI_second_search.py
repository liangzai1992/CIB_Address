    ...
    if temp['status'] == 0:
        ...
        for line in result:
            ...
            if 'location' in line:
                ...
                lat = ''
                lng = ''
            data = str(name) + "^" + str(subname) + "^" + str(city) + "^" + str(area) + "^" + str(addr) + "^" + str(lat) + "^" + str(lng)
            try:
                new_txt.write(data + "\n")  # 写入txt
                new_txt.flush()
                count = 1
...
def run(filepath, ak):
    ...
    for i in range(nrows):
        ...
        if num.isspace() or street.isspace(): #如果路名或路号为空，则将名字和行政区域输入百度地图，返回相关条目作为此地址的补充
            ...
            cur = BaiduAPI_singleSearch(keyword, district, ak)
            success_count = success_count + cur
