...
def reverseLng(lng, lat,ak):   #经纬度反向解析
    ...
    return add_list, district, formatted_address,street
...
def run(road,city,ak):
    ...
    for i in road_split[:-1]:
        road2 += i + "\\"
    road_data = road2 + "%s_路库.txt" % (city)
    road_data2 = road2 + "%s_路库_error.txt" % (city)
    ...
    for line in file:
        try:
            ...
            break
        try:
            add_list, district, formatted_address,street = reverseLng(lng, lat,ak)
        except Exception as e:
...
if __name__ == "__main__":
    road = "/Users/ake/Downloads/python/安居客小区数据/成都/成都.txt"  #前期已经爬取下来的小区信息
    city = '成都' #用于生成路库文件
    ak = 'y0j1hsIBRInpjyXub9dFLwsHjnTx73m4'  #百度地图API的ak，建议一个人自己注册一个
    run(road, city, ak)
