...
def reverseLng(name,lng, lat,ak):   #经纬度反向解析   经度在前 纬度在后
    ...
    if str(res[:29]) == 'renderReverse&&renderReverse(':
        ...
        if temp['status'] == 301 or temp['status'] == 302 or temp['status'] == 401 or temp['status'] == 402:
            ...
            print("-----------------等待3s-------------------")
            time.sleep(3)
            ak1=reverseLng(name,lng, lat,ak1)
    return ak1
...
def reverseLng1(name,lng, lat,apartment,ak):   #经纬度反向解析   经度在前 纬度在后
    ...
    if str(res[:29]) == 'renderReverse&&renderReverse(':
        ...
        if temp['status'] == 301 or temp['status'] == 302 or temp['status'] == 401 or temp['status'] == 402:
            ...
            print("-----------------等待3s-------------------")
            time.sleep(3)
            ak1=reverseLng1(name,lng, lat,apartment,ak1)
    return ak1
            
def read_Company_Info(ak):  #读取所有商家信息
    data = xlrd.open_workbook("D:\长沙_公司_地址.xlsx")  # 文件名以及路径
    list_lat_lnt=[]
    global save_list
...
if __name__ == "__main__":
    global road_file
    global error_list
    road_file = open("D:\\长沙_公司_标准路.txt",'a+',encoding='utf8')
    error_list = open("D:\\长沙_公司_标准路error.txt",'a+',encoding='utf8')
    ak = 'lN8FZ9Y8dXeGVdv7aTKvdcnkpXUuMcpQ'
    initial_AK_pond()
